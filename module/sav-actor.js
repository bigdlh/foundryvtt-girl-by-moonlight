...
export class SaVActor extends Actor {
  ...
  rollActionPopup(attribute_name) {
    ...
    if (base_dice > 2) {
      mundane_dice = 2;
      trans_dice = base_dice-mundane_dice;
    ...
    if( dropdowns ) {
      ...
      new Dialog( {
        ...
        <div id="skill-roll">
          ...
          <form>
            ...
            <div class="form-group roll base-dice">
              <label class="base-dice">${ game.i18n.localize( 'BITD.BaseDice' ) }: </label>
			        <label>${ base_dice- trans_dice }d</label>
            </div>
            <div class="form-group roll mod">
              <label>${ game.i18n.localize( 'BITD.Modifier' ) }:</label>
              <select id="mod" name="mod" data-base-dice="${ base_dice }">
            ...
            <div class="form-group roll ">
              <label class="trans-dice">${ game.i18n.localize( 'BITD.TransDice' ) }: </label>
			        <label>${ trans_dice }d Required Transcendence</label>
            </div>
		        <div class="form-group roll total-rolled">
              <label class="total-rolled">${ game.i18n.localize( 'BITD.TotalDice' ) }: </label>
			        <label>${ total_dice }d</label>
      ...
      `,
        buttons: {
          yes: {
            icon: "<i class='fas fa-check'></i>",
            label: game.i18n.localize( 'BITD.Mundane' ),
            callback: async( html ) => {
              let modifier = parseInt( html.find( '[name="mod"]' )[0].value ) - trans_dice;
          ...
          yes2: {
            icon: "<i class='fas fa-check'></i>",
            label: game.i18n.localize( 'BITD.Transcend' ),
            callback: async( html ) => {
              let modifier = parseInt( html.find( '[name="mod"]' )[0].value );
              let position = $( 'input:radio[name="pos"]:checked' )[0].value;
              let effect = $( 'input:radio[name="fx"]:checked' )[0].value;
